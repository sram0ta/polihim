<?php
/**
 * Шаблон для страницы поиска с фильтром по post_type
 * @package ava_theme
 */

get_header();

$allowed_post_types = ['tare', 'lubricants', 'infocenter', 'page'];
    ...
    <div class="container container-main__inner">
        <div class="upper-information grid-12">
            ...
            <div class="breadcrumbs">
                ...
                <div class="breadcrumbs__item active p1">
                    <?= is_search() ? pll__('Результаты поиска') : pll__('Все материалы'); ?>
                </div>
            </div>
        </div>
        <div class="search">
            <h2 class="search__title h2">
                <?= is_search() ? get_search_query() : pll__('Все материалы'); ?>
            </h2>
            <p class="search__counter p1">
                <?= pll__('Найдено:'); ?> <?= $query->found_posts; ?>
            </p>
            <div class="search__list">
                ...
                <?php else : ?>
                    <p class="search__item__counter p1">
                        <?= is_search()
                            ? pll__('Ничего не найдено по запросу:') . ' "' . get_search_query() . '"'
                            : pll__('Постов не найдено');
                        ?>
                    </p>
                <?php endif; ?>
            </div>
